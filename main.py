...
window_size = [480, 360]
window = pygame.display.set_mode(window_size) # once todo fixed in line 88 add RESIZABLE
clock = pygame.time.Clock()
font = pygame.font.SysFont("Arial", 16)

def update_fps(fps):
    color = "green" if fps>30 else "yellow" if fps>10 else "red"
    fps_text = font.render(str(fps), 1, pygame.Color(color))
    return fps_text

while 1:
    events = pygame.event.get()
    fps = round(float(clock.get_fps()), 2)
    for event in events:
        """
    ...
    if fps:
        # print(f"fps: {fps}")
        window.blit(update_fps(fps), (0, 0))
